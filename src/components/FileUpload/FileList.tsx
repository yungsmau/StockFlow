          ...
          {referenceFiles.length > 0 ? (
            referenceFiles.map((file, index) => (
              <FileCard
                ...
                onRemove={(e) => {
                  e.stopPropagation(); // Не открывать выбор файла при удалении
                  onRemove(index, true);
                }}
              />
            ))
            ...
            <div className="file-slot__placeholder">
              <span>Загрузите справочник</span>
            </div>
          )}
        </div>
      </div>
      ...
      >
        <div className="file-slot__content">
          {dataFiles.length > 0 ? (
            ...
            <div className="file-slot__placeholder">
              <span>Загрузите данные остатков</span>
            </div>
          )}
        </div>
      </div>
...
interface FileCardProps {
  file: FileItem;
  onRemove: (e: React.MouseEvent) => void; // ← Добавлен event
}
  ...
  return (
    <div className={`uploaded-file-card ${file.isReference ? 'uploaded-file-card--reference' : ''}`}>
      ...
      <button
        ...
        onClick={onRemove} // ← Event передаётся автоматически
      >
